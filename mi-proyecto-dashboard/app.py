...
# 2. MOTOR DE CARGA DE DATOS (Optimizado para cabeceras reales de VENTA AGENCIAS.xlsx)
@st.cache_data(ttl=120)
def cargar_y_procesar_datos():
    carpeta_data = "DATA"
    # Buscar el archivo específico solicitado
    archivo_objetivo = os.path.join(carpeta_data, "VENTA AGENCIAS.xlsx")
    
    if not os.path.exists(archivo_objetivo):
        # Fallback si no está en DATA/ pero sí en la raíz
        if os.path.exists("VENTA AGENCIAS.xlsx"):
            archivo_objetivo = "VENTA AGENCIAS.xlsx"
        else:
            return None
...
if data_bundle is not None:
    ...
    with col_f4:
        ...
        agencia_sel = st.multiselect("4. Agencia(s):", agencias_disp, default=agencias_disp)
    ...
    df_f4 = df_f3[df_f3['Agencia Descripción'].isin(agencia_sel)]
    ...
    # =========================================================================
    # --- SECCIÓN 2: DESEMPEÑO OPERATIVO FINANCIERO (VINCULADO TOTALMENTE AL FILTRO)
    # =========================================================================
    ...
    # =========================================================================
    # --- SECCIÓN 3: MAPA GEOGRÁFICO (AHORA TOTALMENTE FILTRADO POR MES)
    # =========================================================================
    st.markdown("---")
    ...
    # --- SECCIÓN 5: TABLA DE AUDITORÍA BASE DE DATOS (Fecha de Emisión Limpia) ---
    st.markdown("---")
    st.markdown("### 📋 Vista Consolidada de Registros")
    with st.expander("Ver base de datos de transacciones filtradas"):
        st.dataframe(df, use_container_width=True)
else:
    st.warning("⚠️ No se encontró el archivo 'VENTA AGENCIAS.xlsx' en el directorio.")